  ...
  return (
    <section className="section py-20">
      <div className="container-custom">
        ...
        <div className="relative ml-4 sm:ml-6 space-y-8 pl-6 sm:pl-10" style={{ borderLeft: '2px solid hsl(var(--primary) / 0.2)' }}>
          {experience.map((exp, i) => (
            <FadeIn key={exp.id} delay={i * 0.12} direction="left">
              <div className="relative group">
                {/* Timeline Dot Icon */}
                <div className="absolute -left-[41px] sm:-left-[61px] top-6 w-9 h-9 sm:w-10 sm:h-10 rounded-2xl flex items-center justify-center text-white group-hover:scale-110 transition-transform duration-300 z-10" style={{ background: 'linear-gradient(135deg, hsl(var(--primary)), hsl(var(--secondary)))', boxShadow: '0 4px 16px hsl(var(--primary) / 0.3)', border: '2px solid #000' }}>
                  <Briefcase size={16} />
                </div>

                {/* Card Container */}
                <div className="glass-card p-6 sm:p-8 border border-white/10 rounded-3xl transition-all duration-300" onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.borderColor='hsl(var(--primary) / 0.3)'}} onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.borderColor='rgba(255,255,255,0.1)'}}>
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-4 pb-4 border-b border-white/[0.08]">
                    <div className="space-y-1">
                      ...
                      <p className="font-semibold text-sm sm:text-base flex items-center gap-1.5" style={{ color: 'hsl(var(--primary))' }}>
