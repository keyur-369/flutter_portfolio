...
function ExpForm({ item, onClose, onSave }: { item?: Experience | null; onClose: () => void; onSave: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-16 bg-black/70 backdrop-blur-sm overflow-y-auto" onClick={onClose}>
      <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="glass-card p-8 w-full max-w-lg my-4" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between mb-6">
          <h2 className="font-display font-bold text-2xl text-white">{item ? 'Edit Experience' : 'Add Experience'}</h2>
          <button onClick={onClose} className="p-2 rounded-xl glass text-white/60 hover:text-white"><X size={16} /></button>
        </div>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div><label className="text-sm text-white/60 mb-1 block">Company *</label>
              <input {...register('company', { required: true })} className="input-glass" placeholder="Patrixel" /></div>
            <div><label className="text-sm text-white/60 mb-1 block">Role *</label>
              <input {...register('role', { required: true })} className="input-glass" placeholder="Flutter Developer Intern" /></div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="text-sm text-white/60 mb-1 block">Employment Type</label>
            ...
            <div><label className="text-sm text-white/60 mb-1 block">Location</label>
              <input {...register('location')} className="input-glass" placeholder="Gujarat, India" /></div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div><label className="text-sm text-white/60 mb-1 block">Start Date</label>
              <input {...register('start_date')} type="date" className="input-glass" /></div>
            <div><label className="text-sm text-white/60 mb-1 block">End Date</label>
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div><h1 className="font-display font-black text-3xl text-white mb-1">Experience</h1>
...
<p className="text-xs text-white/40">
  {formatDate(exp.start_date ?? null)} — {exp.currently_working ? 'Present' : formatDate(exp.end_date ?? null)}
          ...
          {items.length === 0 && <div className="text-center py-12 glass-card"><p className="text-white/40 mb-3">No experience entries</p><button onClick={() => setShowForm(true)} className="btn-primary text-sm"><Plus size={14} /> Add</button></div>}
        </div>
      )}
      <AnimatePresence>
