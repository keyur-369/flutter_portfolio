...
function BlogForm({ item, onClose, onSave }: { item?: Blog | null; onClose: () => void; onSave: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-8 bg-black/70 backdrop-blur-sm overflow-y-auto" onClick={onClose}>
      <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="glass-card p-8 w-full max-w-2xl my-4" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between mb-6">
          <h2 className="font-display font-bold text-2xl text-white">{item ? 'Edit Post' : 'New Post'}</h2>
          <button onClick={onClose} className="p-2 rounded-xl glass text-white/60 hover:text-white"><X size={16} /></button>
        </div>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div><label className="text-sm text-white/60 mb-1 block">Title *</label><input {...register('title', { required: true })} className="input-glass" placeholder="Blog post title" /></div>
          <div><label className="text-sm text-white/60 mb-1 block">Slug</label><input {...register('slug')} className="input-glass" placeholder="blog-post-slug" /></div>
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div><h1 className="font-display font-black text-3xl text-white mb-1">Blogs</h1>
